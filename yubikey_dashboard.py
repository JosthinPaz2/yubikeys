...
class YubiDash(ctk.CTk):
    def __init__(self):
        ...
        self.btn_nueva = ctk.CTkButton(self.sidebar, text="Register New", command=self.show_nueva_panel, corner_radius=15)
        self.btn_nueva.pack(pady=8, padx=20, fill="x")
        
        self.btn_ingreso = ctk.CTkButton(self.sidebar, text="Break/Lunch", command=self.show_ingreso_panel, corner_radius=15)
        self.btn_ingreso.pack(pady=8, padx=20, fill="x")
        
        self.btn_asignacion = ctk.CTkButton(self.sidebar, text="Assign/Return", command=self.show_asignacion_panel, corner_radius=15)
        self.btn_asignacion.pack(pady=8, padx=20, fill="x")
        
        self.btn_perdida = ctk.CTkButton(self.sidebar, text="Loss/Damage", command=self.show_perdida_panel, corner_radius=15)
        self.btn_perdida.pack(pady=8, padx=20, fill="x")
        
        self.btn_inv = ctk.CTkButton(self.sidebar, text="INVENTORY", fg_color="transparent", border_width=1, command=self.show_inv_view, corner_radius=15)
        self.btn_inv.pack(pady=8, padx=20, fill="x")
        
        self.btn_report = ctk.CTkButton(self.sidebar, text="REPORTS", fg_color="transparent", border_width=1, command=self.show_report_view, corner_radius=15)
    ...
    def setup_nueva_panel(self):
        ctk.CTkLabel(self.panel_nueva, text="Register New Yubikey", font=("Inter", 22, "bold")).pack(pady=30)
        self.entry_nueva = ctk.CTkEntry(self.panel_nueva, placeholder_text="Scan new serial", width=300, height=40, font=("Inter", 16), border_width=3, border_color=BORDER_DEFAULT, corner_radius=15)
        self.entry_nueva.pack(pady=10)
        self.entry_nueva.bind("<Return>", self.registrar_nueva_yubikey)
        self.label_nueva = ctk.CTkLabel(self.panel_nueva, text="", text_color="red", font=("Inter", 13, "bold"))
        self.label_nueva.pack(pady=5)
        self.setup_recent_table(self.panel_nueva, 'nueva')

    def setup_ingreso_panel(self):
        ctk.CTkLabel(self.panel_ingreso, text="Break / Lunch Check-in/out", font=("Inter", 22, "bold")).pack(pady=30)
        ctk.CTkLabel(self.panel_ingreso, text="Only for yubikeys IN USE", font=("Inter", 13), 
                    text_color="#94a3b8").pack(pady=(0, 20))
        self.entry_ingreso = ctk.CTkEntry(self.panel_ingreso, placeholder_text="Scan for Check-in/out", width=300, height=40, font=("Inter", 16), border_width=3, border_color=BORDER_DEFAULT, corner_radius=15)
        self.entry_ingreso.pack(pady=10)
        self.entry_ingreso.bind("<Return>", lambda e: self.process_session_scan('ingreso_salida'))
        self.label_ingreso = ctk.CTkLabel(self.panel_ingreso, text="", text_color="red", font=("Inter", 13, "bold"))
        self.label_ingreso.pack(pady=5)
        self.setup_recent_table(self.panel_ingreso, 'ingreso')

    def setup_asignacion_panel(self):
        ctk.CTkLabel(self.panel_asignacion, text="Assign / Return", font=("Inter", 22, "bold")).pack(pady=30)
        self.entry_asignacion = ctk.CTkEntry(self.panel_asignacion, placeholder_text="Scan to Assign/Return", width=300, height=40, font=("Inter", 16), border_width=3, border_color=BORDER_DEFAULT, corner_radius=15)
        self.entry_asignacion.pack(pady=10)
        self.entry_asignacion.bind("<Return>", lambda e: self.process_session_scan('asignacion_retoma'))
        self.label_asignacion = ctk.CTkLabel(self.panel_asignacion, text="", text_color="red", font=("Inter", 13, "bold"))
        self.label_asignacion.pack(pady=5)
        self.setup_recent_table(self.panel_asignacion, 'asignacion')

    def setup_perdida_panel(self):
        ctk.CTkLabel(self.panel_perdida, text="Loss / Damage Report", font=("Inter", 22, "bold")).pack(pady=30)
        ctk.CTkLabel(self.panel_perdida, text="Scan serial or enter Pipkins code", font=("Inter", 13), 
                    text_color="#94a3b8").pack(pady=(0, 20))
        self.entry_perdida = ctk.CTkEntry(self.panel_perdida, placeholder_text="Scan serial or enter Pipkins code", width=300, height=40, font=("Inter", 16), border_width=3, border_color=BORDER_DEFAULT, corner_radius=15)
        self.entry_perdida.pack(pady=10)
        self.entry_perdida.bind("<Return>", lambda e: self.process_session_scan('perdida_dano'))
        self.label_perdida = ctk.CTkLabel(self.panel_perdida, text="", text_color="red", font=("Inter", 13, "bold"))
    ...
    def setup_recent_table(self, parent, tipo):
        ...
        ctk.CTkLabel(frame, text="Recent movements", font=("Inter", 15, "bold"), text_color="#38bdf8").pack(anchor='w', padx=10, pady=(8, 0))
        
        columns = ("Serial", "Pipkins Code", "Action", "Date", "Time")
        style = ttk.Style()
        style.theme_use('clam')
        style.configure(f'{tipo}.Treeview', font=('Segoe UI', 11), rowheight=28, background=SLATE_GRAY, fieldbackground=SLATE_GRAY, foreground="#e0e7ef", borderwidth=0)
        style.configure(f'{tipo}.Treeview.Heading', font=('Segoe UI', 12, 'bold'), background="#334155", foreground="#38bdf8")
        
        tree = ttk.Treeview(frame, columns=columns, show='headings', height=5, style=f'{tipo}.Treeview')
        for col in columns:
    ...
    def ask_user_and_pipkins(self):
        ...
        ctk.CTkLabel(frame, text="Manual Registration", font=("Inter", 24, "bold"), text_color="#38bdf8").pack(pady=(10, 18))
        
        ctk.CTkLabel(frame, text="Yubikey Serial:", font=("Inter", 16)).pack(anchor='w', padx=10, pady=(4, 0))
        serial_var = ctk.StringVar()
        serial_entry = ctk.CTkEntry(frame, textvariable=serial_var, width=260, font=("Inter", 15), corner_radius=10)
        serial_entry.pack(pady=(0, 10))
        serial_entry.focus_set()
        
        ctk.CTkLabel(frame, text="User:", font=("Inter", 16)).pack(anchor='w', padx=10, pady=(4, 0))
        usuario_var = ctk.StringVar()
        usuario_entry = ctk.CTkEntry(frame, textvariable=usuario_var, width=260, font=("Inter", 15), corner_radius=10)
        usuario_entry.pack(pady=(0, 10))
        
        ctk.CTkLabel(frame, text="Pipkins Code:", font=("Inter", 16)).pack(anchor='w', padx=10, pady=(4, 0))
        pipkins_var = ctk.StringVar()
        pipkins_entry = ctk.CTkEntry(frame, textvariable=pipkins_var, width=260, font=("Inter", 15), corner_radius=10)
        pipkins_entry.pack(pady=(0, 10))
        
        error_label = ctk.CTkLabel(frame, text="", text_color="#ef4444", font=("Inter", 13, "bold"))
        ...
        ctk.CTkButton(frame, text="Register", command=registrar, fg_color="#38bdf8", hover_color="#0ea5e9", font=("Inter", 17, "bold"), corner_radius=12).pack(pady=16)
        modal.wait_window()
    ...
    def ask_break_type(self):
        ...
        modal.grab_set()
        ctk.CTkLabel(modal, text="Check-in or check-out from?", font=("Inter", 17, "bold"), text_color="#38bdf8").pack(pady=(18, 18))
        tipo_var = ctk.StringVar(value="Break")
        opciones = ["Break", "Lunch"]
        for op in opciones:
            ctk.CTkRadioButton(modal, text=op, variable=tipo_var, value=op, font=("Inter", 15)).pack(anchor='w', padx=40, pady=8)
        result = {'ok': False}
        def aceptar():
            result['ok'] = True
            modal.destroy()
        ctk.CTkButton(modal, text="Continue", command=aceptar, fg_color="#38bdf8", hover_color="#0ea5e9", font=("Inter", 15, "bold"), width=150, corner_radius=10).pack(pady=18)
        modal.wait_window()
        if result['ok']:
            return tipo_var.get()
        return None
    ...
    def ask_asignacion_retoma_type(self):
        ...
        ctk.CTkLabel(modal, text="What action will you perform?", font=("Inter", 18, "bold"), text_color="#38bdf8").pack(pady=(20, 15))
        
        ctk.CTkLabel(modal, text="• Assign: Give AVAILABLE yubikey to new user", 
                    font=("Inter", 13), text_color="#94a3b8", justify="left").pack(pady=8, padx=30, anchor="w")
        ...
        def aceptar():
            result['ok'] = True
            modal.destroy()
        ctk.CTkButton(modal, text="Continue", command=aceptar, fg_color="#38bdf8", 
                     hover_color="#0ea5e9", font=("Inter", 15, "bold"), 
                     width=200, corner_radius=12).pack(pady=25)
        modal.wait_window()
        if result['ok']:
            return tipo_var.get()
        return None
    ...
    def ask_nuevo_usuario_pipkins(self):
        ...
        ctk.CTkLabel(frame, text="Assign to new user", font=("Inter", 20, "bold"), 
                    text_color="#38bdf8").pack(pady=(10, 18))
        
        ctk.CTkLabel(frame, text="User name:", font=("Inter", 15)).pack(anchor='w', padx=10, pady=(4, 0))
        usuario_var = ctk.StringVar()
        usuario_entry = ctk.CTkEntry(frame, textvariable=usuario_var, width=260, 
                                    font=("Inter", 14), corner_radius=10)
        usuario_entry.pack(pady=(0, 10))
        usuario_entry.focus_set()
        
        ctk.CTkLabel(frame, text="Pipkins Code:", font=("Inter", 15)).pack(anchor='w', padx=10, pady=(4, 0))
        pipkins_var = ctk.StringVar()
        pipkins_entry = ctk.CTkEntry(frame, textvariable=pipkins_var, width=260, 
                                    font=("Inter", 14), corner_radius=10)
        pipkins_entry.pack(pady=(0, 10))
        ...
        ctk.CTkButton(frame, text="Assign", command=aceptar, fg_color="#38bdf8", 
                     hover_color="#0ea5e9", font=("Inter", 16, "bold"), 
    ...
    def ask_comentario(self, titulo, mensaje):
        ...
        ctk.CTkLabel(frame, text=mensaje, font=("Inter", 16, "bold"), 
                    text_color="#38bdf8", wraplength=420).pack(pady=(10, 10))
        
        comentario_text = ctk.CTkTextbox(frame, width=430, height=150, font=("Inter", 13))
        ...
        ctk.CTkButton(btn_frame, text="Submit", command=aceptar, fg_color="#38bdf8", 
                     hover_color="#0ea5e9", font=("Inter", 15, "bold"), 
        ...
        ctk.CTkButton(btn_frame, text="Skip", command=modal.destroy, 
                     ...
                     font=("Inter", 15), corner_radius=12, width=100).pack(side="left", padx=10)
        ...
        for item in datos:
            if item['serial'].strip().upper() == serial.upper():
                ...
                if session == 'ingreso_salida':
                    ...
                    modal.grab_set()
                    ctk.CTkLabel(modal, text="What type of incident is this?", font=("Inter", 17, "bold"), 
                    ...
                    def aceptar_modal():
                        ...
                        modal.destroy()
                    ctk.CTkButton(modal, text="Continue", command=aceptar_modal, fg_color="#38bdf8", 
                                 hover_color="#0ea5e9", font=("Inter", 15, "bold"),
    ...
    def setup_inv_view(self):
        ctk.CTkLabel(self.view_inv, text="General Inventory", font=("Inter", 32, "bold"), text_color="#38bdf8").pack(pady=15)
        
        filter_frame = ctk.CTkFrame(self.view_inv, fg_color=SLATE_GRAY, corner_radius=12)
        filter_frame.pack(pady=10, padx=20, fill="x")
        
        ctk.CTkLabel(filter_frame, text="Filter by state:", font=("Inter", 14, "bold")).pack(side="left", padx=15, pady=10)
        
        self.filter_var = ctk.StringVar(value="All")
        estados = ["All", "Available", "In Use", "On Break", "On Lunch", "Loss", "Damage"]
        for estado in estados:
            ctk.CTkRadioButton(filter_frame, text=estado, variable=self.filter_var, value=estado,
                             command=self.filter_inventory, font=("Inter", 13)).pack(side="left", padx=8, pady=10)
        
        search_frame = ctk.CTkFrame(self.view_inv, fg_color="transparent")
        search_frame.pack(pady=5)
        self.search_var = ctk.StringVar()
        search_entry = ctk.CTkEntry(search_frame, textvariable=self.search_var, 
                                   placeholder_text="Search by serial or user...", width=300)
        search_entry.pack(side="left", padx=10)
        search_entry.bind("<KeyRelease>", lambda e: self.filter_inventory())
        
        ctk.CTkButton(search_frame, text="📋 View Details", command=self.show_item_details,
                     fg_color="#38bdf8", hover_color="#0ea5e9", font=("Inter", 13, "bold")).pack(side="left", padx=10)
        ...
        self.inv_tree = ttk.Treeview(table_frame, columns=cols, show='headings', style='Custom.Treeview')
        
        style = ttk.Style()
        style.configure('Custom.Treeview', font=('Segoe UI', 12), rowheight=35, background=SLATE_GRAY, 
                       fieldbackground=SLATE_GRAY, foreground="white")
        ...
        scrollbar = ttk.Scrollbar(self.inv_tree, orient="vertical", command=self.inv_tree.yview)
    ...
    def show_item_details(self):
        ...
        if not selected:
            ...
            ctk.CTkLabel(modal, text="⚠️ Select a yubikey first", 
                        font=("Inter", 16, "bold"), text_color="orange").pack(pady=40)
        ...
        ctk.CTkLabel(header, text=f"Serial: {serial}", font=("Inter", 20, "bold"), 
        ...
        ctk.CTkLabel(main_frame, text="📋 Complete History", font=("Inter", 16, "bold"), 
        ...
        if not historial:
            ctk.CTkLabel(scroll_frame, text="No history registered", 
                        font=("Inter", 13), text_color="#94a3b8").pack(pady=20)
        else:
            for i, h in enumerate(reversed(historial)):
                ...
                ctk.CTkLabel(header_hist, text=f"{h['fecha']} {h['hora']}", 
                            font=("Inter", 11), text_color="#94a3b8").pack(side="right")
                
                if h.get('usuario'):
                    ctk.CTkLabel(item_frame, text=f"User: {h['usuario']}", 
                                font=("Inter", 12), text_color="#cbd5e1").pack(anchor="w", padx=10, pady=(0, 3))
                ...
                if h.get('comentario'):
                    ctk.CTkLabel(item_frame, text=f"💬 {h['comentario']}", 
                                font=("Inter", 11), text_color="#fbbf24", 
    ...
    def setup_report_view(self):
        """Setup reports view with tabs for Loss and Damage"""
        ctk.CTkLabel(self.view_report, text="📊 INCIDENT REPORTS", font=("Inter", 32, "bold"), 
    ...
    def setup_loss_report_tab(self):
        """Setup the Loss report tab"""
        ctk.CTkLabel(self.tab_loss, text="Lost Yubikeys Report", font=("Inter", 20, "bold"), 
        ...
        self.loss_count_label = ctk.CTkLabel(stats_frame, text="Total Lost: 0", 
                                            font=("Inter", 16, "bold"), text_color="#ef4444")
        ...
        self.loss_tree = ttk.Treeview(table_frame, columns=cols, show='headings', height=8)
        
        style = ttk.Style()
        style.configure('Loss.Treeview', font=('Segoe UI', 11), rowheight=30, 
                       background=SLATE_GRAY, fieldbackground=SLATE_GRAY, foreground="white")
        style.configure('Loss.Treeview.Heading', font=('Segoe UI', 12, 'bold'), 
                       background="#7f1d1d", foreground="#ef4444")
        ...
        ctk.CTkLabel(comment_frame, text="📝 Incident Description", font=("Inter", 14, "bold"), 
                    text_color="#ef4444").pack(anchor="w", padx=10, pady=5)
        
        self.loss_comment_text = ctk.CTkTextbox(comment_frame, height=100, font=("Inter", 12))
        ...
        ctk.CTkButton(self.tab_loss, text="📥 Export to CSV", command=self.export_loss_report,
                     fg_color="#ef4444", hover_color="#dc2626", 
                     font=("Inter", 14, "bold")).pack(pady=10)

    def setup_damage_report_tab(self):
        """Setup the Damage report tab"""
        ctk.CTkLabel(self.tab_damage, text="Damaged Yubikeys Report", font=("Inter", 20, "bold"), 
        ...
        self.damage_count_label = ctk.CTkLabel(stats_frame, text="Total Damaged: 0", 
                                              font=("Inter", 16, "bold"), text_color="#8b5cf6")
        ...
        cols = ("Serial", "User", "Pipkins", "Date", "Reported By")
        self.damage_tree = ttk.Treeview(table_frame, columns=cols, show='headings', height=8)
        
        style = ttk.Style()
        style.configure('Damage.Treeview', font=('Segoe UI', 11), rowheight=30, 
                       background=SLATE_GRAY, fieldbackground=SLATE_GRAY, foreground="white")
        style.configure('Damage.Treeview.Heading', font=('Segoe UI', 12, 'bold'), 
                       background="#5b21b6", foreground="#8b5cf6")
        ...
        ctk.CTkLabel(comment_frame, text="📝 Damage Description", font=("Inter", 14, "bold"), 
                    text_color="#8b5cf6").pack(anchor="w", padx=10, pady=5)
        
        self.damage_comment_text = ctk.CTkTextbox(comment_frame, height=100, font=("Inter", 12))
        ...
        ctk.CTkButton(self.tab_damage, text="📥 Export to CSV", command=self.export_damage_report,
                     fg_color="#8b5cf6", hover_color="#7c3aed", 
                     font=("Inter", 14, "bold")).pack(pady=10)

    def setup_summary_tab(self):
        """Setup the summary tab"""
        ctk.CTkLabel(self.tab_summary, text="📊 Incident Summary", font=("Inter", 20, "bold"), 
        ...
        ctk.CTkLabel(loss_card, text="🔴 Total Losses", font=("Inter", 16, "bold")).pack(pady=10)
        self.summary_loss_label = ctk.CTkLabel(loss_card, text="0", 
                                              font=("Inter", 36, "bold"), text_color="#ef4444")
        ...
        ctk.CTkLabel(damage_card, text="🟣 Total Damages", font=("Inter", 16, "bold")).pack(pady=10)
        self.summary_damage_label = ctk.CTkLabel(damage_card, text="0", 
                                                font=("Inter", 36, "bold"), text_color="#8b5cf6")
        ...
        ctk.CTkLabel(total_card, text="📋 Total Incidents", font=("Inter", 16, "bold")).pack(pady=10)
        self.summary_total_label = ctk.CTkLabel(total_card, text="0", 
                                               font=("Inter", 36, "bold"), text_color="#3b82f6")
        ...
        ctk.CTkLabel(recent_frame, text="🕐 Recent Incidents", font=("Inter", 16, "bold"), 
                    text_color="#38bdf8").pack(anchor="w", padx=15, pady=10)
        
        self.recent_incidents_text = ctk.CTkTextbox(recent_frame, height=200, font=("Inter", 11))
